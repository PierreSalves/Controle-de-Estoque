...
{
    public partial class frmCadastroRawMaterial : Form
    {
        ...
        public frmCadastroRawMaterial()
        {
            InitializeComponent();
            ExibirDados();
            edtDesc.Focus();
        }
        private void frmCadastroRawMaterial_Load(object sender, EventArgs e)
        {
        ...
        private void ExibirDados()
        {
            try
            {
                ...
                adapt = new SqlDataAdapter(@"SELECT
                                                    ...
                                                    tipo AS Tipo_de_Material,
                                                    unidade AS Unidade,
                                                    qt_estoque AS Quantidade_em_Estoque,
                                                    data_criacao AS Data_de_Criação
                                               FROM tblRawMaterial", sqlcon);
                adapt.Fill(dataTable);
                dGridView_Material.DataSource = dataTable;
            }
            catch (Exception ex)
            {
        ...
        private void imgButton_Save_Click(object sender, EventArgs e)
        {
            if (INSERT)
            {
                ...
                {
                    string insert = @"INSERT INTO[dbo].[tblRawMaterial]
                                                      ([descricao],
                                                      [tipo],
                                                      [unidade],
                                                      [data_criacao])
            ...
            if (UPDATE)
            {
                if (edtDesc.Text != "" && edtTipo.Text != "" && edtUnit.Text != "")
                {
                    string update = @"UPDATE [dbo].[tblRawMaterial]
                                         SET
                                                      [descricao] = @descricao,
                                                      [tipo] = @tipo,
                                                      [unidade] = @unidade
                    ...
                    try
                    {
                        if (DialogResult.Yes == MessageBox.Show("Tem certeza que deseja alterar o registro?", "Confirmação", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2))
                        {
                            ...
                            cmnd.Parameters.Add("@id", SqlDbType.Int).Value = dGridView_Material.SelectedRows[0].Cells[0].Value;
                            cmnd.Parameters.Add("@descricao", SqlDbType.VarChar).Value = edtDesc.Text;
                            cmnd.Parameters.Add("@tipo", SqlDbType.VarChar).Value = edtTipo.Text;
                            cmnd.Parameters.Add("@unidade", SqlDbType.VarChar).Value = edtUnit.Text;
            ...
            if (DELETE)
            {
                string delete = "DELETE FROM[dbo].[tblRawMaterial] " +
                                    "WHERE id = @id;";
                try
                {
