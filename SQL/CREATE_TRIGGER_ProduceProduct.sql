...
BEGIN 
	...
	----------------------------------------------------------------------------MATERIALS
	SELECT @qt_Materials_inRecipe = (SELECT COUNT(rawrecipe.id_rawmaterial_fk)
	...
	SELECT @Material_ID =  (SELECT rawrecipe.id_rawmaterial_fk
								FROM tblRaw_Recipe AS rawrecipe
								INNER JOIN tblRawMaterial AS rawmaterial ON rawrecipe.id_rawmaterial_fk = rawmaterial.id
								WHERE rawrecipe.id_recipe_fk = @Recipe_ID);
							
	--SELECT @Equacao_de_Materiais_Usados = @Estoque_qtMaterial - (--material usado)

	--INCOMPLETO
	
	----------------------------------------------------------------------------UPDATE qtMaterials
	WHILE @i < @qt_Materials_inRecipe
	BEGIN
